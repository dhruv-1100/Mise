...
from app.schema import Recipe, extraction_result_adapter
...
@pytest.mark.parametrize(
    ("kind", "name", "data"),
    [(k, n, d) for k in ("recipe", "result") for n, d in _load(k, "valid")],
)
...
@pytest.mark.parametrize(
    ("kind", "name", "data"),
    [(k, n, d) for k in ("recipe", "result") for n, d in _load(k, "invalid")],
)
